 ...
 * One exception is the method setPosition( CubePosition cubePos, boolean test ) which is used once in an instantiation to check for solvability of a cube state.
...
public final class SolverKociemba extends Solver
{
	...
	public boolean setPosition( CubePosition cubePos, boolean test ){
		...
		// in the original program the following true is settings.supergroup (changed to use in genetic solver)
		if( true ){
		...
		newpos[0]=0;
		for( i=10;i>=0; i--)
			newpos[0]=newpos[0]*2+cubePos.cubeletOri[8+i];
