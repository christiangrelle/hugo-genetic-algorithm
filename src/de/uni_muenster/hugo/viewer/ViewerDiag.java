...
public final class ViewerDiag extends Viewer
{
//--- Display routines ---
	...
	/**
	 * Instantiates a new viewer diag.
	 * 
	 * @param x the x
	 * @param y the y
	 ...
	 */
	public ViewerDiag(int x, int y, Settings s, ActionListener m, ViewerDelegate v){
		super(x,y,s,m, v);
		int i,c;
	...
	public void paint(Graphics g)
	{
		...
		for( i=0; i<9; i++){ // do L face
			offGraphics.setColor(colors[settings.cubePos.cubeletPerm[20+settings.cubePos.faceletColor[i]]-20]); // draw facelet
			offGraphics.fillPolygon(faceletsI[i]);
			if( settings.superGroup ){
				offGraphics.setColor(baseColor);
				offGraphics.fillPolygon(faceOriMarks[i][settings.cubePos.faceletOri[i]]);
			}
		}
