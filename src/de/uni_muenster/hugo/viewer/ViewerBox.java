...
//--- Display routines ---
	...
	/** The face orientation marks. */
	Polygon faceOriMarks[][];

	/**
	 * Instantiates a new viewer box.
	 * 
	 * @param x the x
	 * @param y the y
	 * @param s the settings
	 * @param m the actionlistener
	 * @param v the viewdelegate
	 */
	public ViewerBox(int x, int y, Settings s, ActionListener m, ViewerDelegate v){
		super(x,y,s,m, v);
		int i,j,c;
		CELL = x/16;
		BORD = y/16;
	...
	public void paint(Graphics g)
	{
		...
		for( int c=0; c<54; c++){	// each facelet
			...
			if( settings.superGroup ){
				...
				offGraphics.fillPolygon(faceOriMarks[c][settings.cubePos.faceletOri[c]]);
			}
		}
...
//--- mouse routines ---
	/** The moves. Conversion from facelet mouse move to internal move number.*/
final int moves[][] = {
