...
int FACE, CELL, BORD, OFFSETX, OFFSETY;
	...
	/** The face coordinates x. */
	int facecoordX[];
	
	/** The face coordinates y. */
	int facecoordY[];
	
	/**
	 * Instantiates a new viewer flat.
	 * 
	 * @param x the x
	 * @param y the y
	 ...
	 */
	public ViewerFlat(int x, int y, Settings s, ActionListener m, ViewerDelegate v){
		super(x,y,s,m,v);
		CELL = x/12;
		FACE = y/9;
		...
		for( k=0; k<6; k++){	//each face
			...
			for( i=0; i<3; i++){	// each row
				for( j=0; j<3; j++){	// each facelet
					offGraphics.setColor(colors[settings.cubePos.cubeletPerm[20+settings.cubePos.faceletColor[c]]-20]); // draw facelet
...
final int moves[][] = {
	...
	protected int getFacelet( int x, int y )
	{
		...
		for( k=0; k<6; k++){
			...
			{
				f= k*9+ 3* (int)( (y-facecoordY[k])/CELL ) + (int)( (x-facecoordX[k])/CELL );
				return(f);
			}
		}
		return(-1);
	...
	/**
	 * Execute mouse move. Perform move of facelet f in direction x,y.
	 * 
	 * @param f the f
	 * @param x the x
	 * @param y the y
